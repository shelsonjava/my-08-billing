...
import com.info08.billing.callcenter.shared.items.Abonent;
import com.info08.billing.callcenter.shared.items.OrgInfoByPhone;
	...
	public Abonent saveOrUpdateAbonent(DSRequest dsRequest) throws Exception {
		...
		try {
			...
			insertStatement = connection
					.prepareCall("{ call info.newBillSupport2.saveOrUpdateAbonent( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,? ) }");
			...
			if (phonesObject != null) {
				...
				if (phones != null && !phones.isEmpty()) {
					...
					for (Object object : keys) {
						...
						insertStatement = connection
								.prepareCall("{ call info.newBillSupport2.saveOrUpdateAbonentPhones( ?,?,?,?,?,?,?,?,?,? ) }");
