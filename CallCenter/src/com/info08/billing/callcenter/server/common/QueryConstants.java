...
public interface QueryConstants {
	...
	public static final String Q_GET_MAIN_ORGS_PHONES_HIERARCHY_BLOCK_LIST = " select\n" +
			...
			"                         from info.block_list_phones tt\n" + 
	...
	public static final String Q_GET_MAIN_DET_PHONES_HIERARCHY_EXCEPT_BLOCK_LIST = "select /*+ index(p PHN_PRY_KS001)*/ p.phone\n" +
					...
					"       (select t.main_detail_id from info.main_details t\n" + 
	...
	public static final String Q_GET_BLOCK_LIST_PHONES = "select tt.phone from info.block_list_phones tt where tt.block_list_id = ? ";
	...
	public static final String Q_GET_MAIN_DET_PHONES_HIERARCHY = " select /*+ index(p PHN_PRY_KS001)*/ p.phone\n" +
					...
					"where a.main_detail_id in (\n" + 
					"      select\n" + 
					"          t.main_detail_id from info.main_details t\n" + 
					"      start with t.main_detail_id = ? and t.deleted = 0\n" + 
					"      connect by prior t.main_detail_id = t.main_detail_master_id\n" + 
					") and p.deleted = 0 and a.deleted = 0 ";
//	
//	public static final String Q_GET_CALL_CNT_BY_CONT_AND_MAIN_DET_ID = " select /*+ index(ls IDX_LOG_SESS_PHANDDT) */ count(1) from info.log_sessions ls\n" +
//					"       inner join info.log_session_charges ch on ch.session_id = ls.session_id\n" + 
//					"       inner join info.contracts c on c.contract_id = ? \n" + 
...
//					"            select\n" + 
//					"                t.main_detail_id from info.main_details t\n" + 
//					"            start with t.main_detail_id = ((select c.main_detail_id from info.contracts c where c.contract_id = ? )) and t.deleted = 0\n" + 
//					"            connect by prior t.main_detail_id = t.main_detail_master_id\n" + 
//					"      )\n" + 
//					")";
...
//	public static final String Q_GET_CALL_CNT_BY_CONT_AND_MAIN_ID = " select /*+ index(ls IDX_LOG_SESS_PHANDDT) */ count(1) from info.log_sessions ls\n" +
//					"       inner join info.log_session_charges ch on ch.session_id = ls.session_id\n" + 
//					"       inner join info.contracts c on c.contract_id = ?\n" + 
//					"where ch.deleted = 0 and trunc(c.start_date) <= trunc(ls.start_date) and trunc(c.end_date)>=trunc(ls.start_date) and ls.phone in (\n" + 
//					"      select\n" + 
...
//					"               t.main_id\n" + 
//					"            from info.main_services t\n" + 
//					"            start with t.main_id =((select c.main_id from info.contracts c where c.contract_id = ?))  and t.service_id = 3\n" + 
//					"            connect by prior t.main_id = t.main_master_id\n" + 
//					"       )\n" + 
//					")";
	...
	public static final String Q_CHECK_PHONE_MAIN_ORG =  "select count(1) from info.main_services ms\n" +
					"       inner join info.abonents a on a.main_id = ms.main_id\n" + 
					"       inner join info.phones p on p.phone_id = a.phone_id\n" + 
					"where ms.main_id in\n" + 
					"      (select t.main_id from info.main_services t\n" + 
	...
	public static final String Q_CHECK_PHONE_MAIN_ORG_DET =  "select p.phone from info.phones p\n" +
					"       inner join info.abonents a on a.phone_id = p.phone_id\n" + 
					"       inner join info.main_services ms on ms.main_id = a.main_id\n" + 
					"where a.main_detail_id in\n" + 
					"      (select t.main_detail_id from info.main_details t\n" + 
					"       start with t.main_detail_id = ? and t.deleted = 0\n" + 
					"       connect by prior t.main_detail_id = t.main_detail_master_id\n" + 
...
//	public static final String Q_GET_PHONE_LIST_ONLY_CONTR_LIST = "select distinct t.phone from info.phones t\n" +
//					"inner join info.abonents a on a.phone_id = t.phone_id\n" + 
//					"inner join info.main_services ms on ms.main_id = a.main_id\n" + 
...
//					"      select tt.phone from info.contractor_phones tt\n" + 
//					"             inner join info.contracts c on c.contract_id = tt.contract_id\n" + 
...
//					")";
	...
	public static final String Q_GET_PHONE_LIST_ONLY_CONTR_LIST = "select\n" +
					...
					"  )\n" + 
					"  and p.phone not in (\n" + 
					"      select tt.phone from info.contractor_phones tt where tt.contract_id = ? and tt.deleted = 0 " +
					"  )\n";
...
//	public static final String Q_GET_PHONE_LIST_EXCEPT_CONTR_LIST = "select distinct t.phone from info.phones t\n" +
//			"inner join info.abonents a on a.phone_id = t.phone_id\n" + 
//			"inner join info.main_services ms on ms.main_id = a.main_id\n" + 
//			"where t.deleted = 0 and a.deleted = 0 and ms.service_id = 3 and a.main_id = ? and ms.main_id = ?\n" + 
...
//	public static final String Q_GET_PHONE_LIST_ONLY_CONTR_LIST1 = "select distinct p.phone from info.main_services ms\n" +
//			"       inner join info.abonents a on a.main_id = ms.main_id\n" + 
//			"       inner join info.phones p on p.phone_id = a.phone_id\n" + 
//			"where ms.main_id in\n" + 
//			"      (select t.main_id from info.main_services t\n" + 
...
//	public static final String Q_GET_PHONE_LIST_EXCEPT_CONTR_LIST1 = "select distinct p.phone from info.main_services ms\n" +
//			"       inner join info.abonents a on a.main_id = ms.main_id\n" + 
//			"       inner join info.phones p on p.phone_id = a.phone_id\n" + 
//			"where ms.main_id in\n" + 
//			"      (select t.main_id from info.main_services t\n" + 
...
//			")";
	...
	public static final String Q_GET_PHONE_LIST_BY_MAIN_ID = "select distinct p.phone from info.main_services ms\n" +
					"       inner join info.abonents a on a.main_id = ms.main_id\n" + 
					"       inner join info.phones p on p.phone_id = a.phone_id\n" + 
					"where ms.main_id in\n" + 
					"      (select t.main_id from info.main_services t\n" + 
	...
	public static final String Q_GET_PHONE_LIST_BY_MAIN_ID1111 = "select\n" +
					"  distinct p.phone\n" + 
					"from info.main_orgs mo\n" + 
					"inner join info.main_services ms on ms.main_id = mo.main_id\n" + 
					"inner join info.main_details md on md.main_id = mo.main_id\n" + 
					"inner join info.abonents a on a.main_detail_id = md.main_detail_id\n" + 
					"inner join info.phones p on p.phone_id = a.phone_id\n" + 
					"where mo.main_id in\n" + 
					"      (select\n" + 
					"         t.main_id\n" + 
	...
	public static final String Q_DELETE_TELCOMP_IND = "delete from info.tel_comps_ind t where t.tel_comp_id = ? ";
	...
	public static final String Q_GET_CONTRACTOR_ADV_PRICE = " select sum(nvl(t.price,0)) as advPrice from info.contract_price_items t where t.contract_id = ? and t.call_count_start<=? and t.call_count_end > ? ";

	
	public static final String Q_GET_CONTRACTOR_INFO = "select distinct c.contract_id,\n" +
					"       info.date_to_msec(c.start_date) as start_date,\n" + 
					"       info.date_to_msec(c.end_date) as end_date,\n" + 
					"       c.critical_number,\n" + 
					"       c.is_budget,\n" + 
					"       c.price_type,\n" + 
					"       c.price,\n" + 
					...
					"  from info.phones t\n" + 
					" inner join info.abonents a on a.phone_id = t.phone_id\n" + 
					" inner join info.contracts c on c.main_id = a.main_id\n" + 
					" where t.phone = ? and a.deleted = 0 and c.deleted = 0 ";	

	public static final String Q_GET_CALL_CENTER_REQ_MSG = "select t.text from info.call_center_req_msg t where sysdate between t.start_date and t.end_date";
	...
	public static final String Q_GET_OPERATOR_REMARKS = " select count(1)\n" +
					"  from info.log_personell_notes t\n" + 
	...
	public static final String Q_GET_DISC_SMS_FOR_SEND = " SELECT      t.discover_id,\n" +
			...
			"         and not exists (select tt.discovery_id from info.discovery_sms_hist tt\n" + 
	...
	public static final String Q_GET_DISCOVERY_LIST = "select\n" +
			...
			"  t.ccr,\n" + 
			"  t.iscorrect\n" + 
			"from info.discover t\n" + 
			"       inner join info.discover_types tt on tt.discover_type_id = t.discover_type_id\n" + 
			"       left join info.discover_rtypes r on r.response_type_id = t.response_type_id\n" + 
	...
	public static final String Q_GET_TRANSPORT_BY_ID = "select\n"
			...
			+ "\n"
			+ "from info.transports t, transport_types tt,transport_places o, cities oc,transport_places i, cities ic, transport_companies tc, transport_plane tp\n"
			+ "where\n"
	...
	public static final String Q_GET_SESSION_BY_ID = "select * from info.log_sessions t where t.session_id = ? ";
	public static final String Q_GET_PERS_NOTE_BY_ID = "select t.note_id from info.log_personell_notes t where t.note_id = ? ";
	...
	public static final String Q_GET_ORGS_BY_PHONE = "select\n"
			...
			+ "  t.extra_priority,\n"
			+ "  t.new_identcode\n"
			+ "from info.main_orgs t\n"
			+ "     left join info.legal_statuses ls on ls.legal_statuse_id = t.legal_statuse_id\n"
			+ "     left join info.org_partnerbank_lists b on b.main_id = t.partnerbank_id\n"
			+ "where t.main_id in (\n"
			+ "select\n"
			+ "      distinct mo.main_id\n"
			+ "from info.phones t\n"
			+ "inner join info.abonent_to_phones ap on ap.phone_id = t.phone_id\n"
			+ "inner join info.abonents a on a.abonent_id = ap.abonent_id\n"
			+ "inner join info.main_services ms on ms.main_id = a.main_id\n"
			+ "inner join info.main_orgs mo on mo.main_id = a.main_id\n"
			+ "where t.phone = ? and ms.service_id = 3 )";

	public static final String Q_GET_FIRST_NAME_COUNT = " select count(1) from info.firstnames t where t.firstname = ? ";
	public static final String Q_GET_LAST_NAME_COUNT = " select count(1) from info.lastnames t where t.lastname = ? ";
	public static final String Q_GET_FIRST_NAME_COUNT_ALL = " select count(1) from info.firstnames t ";
	public static final String Q_GET_FIRST_NAMES_ALL = " select t.deleted,decode(t.deleted, 0, 'აქტიური', 'გაუქმებული') as deletedText,t.firstname, t.firstname_id,t.rec_date from info.firstnames t where t.deleted=0 order by t.firstname ";
	public static final String Q_GET_LAST_NAMES_ALL = " select t.deleted,decode(t.deleted, 0, 'აქტიური', 'გაუქმებული') as deletedText,t.lastname, t.lastname_id,t.rec_date from info.lastnames t where t.deleted=0 order by t.lastname ";
	public static final String Q_GET_PERS_TYPES_ALL = " select * from info.personnel_types t order by 1 ";
	public static final String Q_GET_PERS_TO_ACC = " select t.access_id from info.personnel_to_access t where t.personnel_id = ? ";
	...
	public static final String Q_GET_STREET_DISTRICTS_ALL = "select t.street_district_id,t.street_id,t.city_region_id,c.city_region_name_geo from info.street_district t\n"
			+ "inner join info.city_regions c on c.city_region_id = t.city_region_id\n"
	...
	// inserts
	public static final String Q_INSERT_PERS_NOTES = "BEGIN insert into info.log_personell_notes t\n"
	...
	public static final String Q_INSERT_SESSION =" insert into info.LOG_SESSIONS (YM, user_name, phone, session_id, call_type, is_new_bill) values (?,?,?,?,?,?) ";
	
	// updates
	public static final String Q_UPDATE_PERS_NOTES = "update info.log_personell_notes t set t.note = ?, t.visible_options = ?, t.particular = ?\n"
			+ "where t.note_id = ? ";

	public static final String Q_UPDATE_FIRST_NAME = "update info.firstnames t set t.firstname = ?, t.upd_user = ? where t.firstname_id = ? ";
	public static final String Q_UPDATE_LAST_NAME = "update info.lastnames t set t.lastname = ?, t.upd_user = ? where t.lastname_id = ? ";

	public static final String Q_UPDATE_FIRST_NAME_STATUS = "update info.firstnames t set t.deleted = ? , t.upd_user = ? where t.firstname_id = ? ";
	public static final String Q_UPDATE_LAST_NAME_STATUS = "update info.lastnames t set t.deleted = ? , t.upd_user = ? where t.lastname_id = ? ";
	...
	public static final String Q_UPDATE_LOCK_STATUS =" update info.lock_status t set t.status = 1 where t.session_id = ? ";
	
	
	

	public static final String Q_UPDATE_SESSION_QUALITY = " update info.log_sessions t set t.session_quality = ? 	where t.session_id = ? ";

	public static final String Q_UPDATE_ABONENT = " update info.abonents t set t.deleted = ?, t.upd_user = ?, t.upd_date = ? where t.abonent_id = ? ";

	public static final String Q_UPDATE_MAIN_SERVICE = " update info.main_services t set t.deleted = ?, t.upd_user = ?, t.upd_date = ? where t.main_id = ? ";

	public static final String Q_UPDATE_MAIN_ADDRESS = " update info.main_address t set t.deleted = ?, t.upd_user = ?, t.upd_date = ? where t.address_id = ? and t.main_id = ? ";

	// public static final String Q_UPDATE_PHONES =
	// " update info.phones t set t.deleted = ?, t.upd_user = ? where t.abonent_id = ? ";

	public static final String Q_UPDATE_PERSONNEL = " update info.personnel t set\n"
			+ "       t.personnel_name = ?,\n"
			+ "       t.personnel_surname = ?,\n"
			+ "       t.user_name = ?,\n"
			+ "       t.password = ?,\n"
	...
	public static final String Q_UPDATE_COUNTRY = "update info.countries t set\n"
			+ "       t.country_name_geo = ?,\n"
			+ "       t.country_name_eng = ?,\n"
			+ "       t.country_code = ?,\n"
			+ "       t.upd_user = ?,\n"
	...
	public static final String Q_UPDATE_PERSONNEL_STATUS = " update info.personnel t set \n"
			+ "       t.deleted = ? ,\n"
			+ "       t.upd_user = ? \n"
			+ "where t.personnel_id = ? ";

	public static final String Q_UPDATE_COUNTRY_STATUS = "update info.countries t set\n"
			+ "       t.deleted = ?,\n"
			+ "       t.upd_user = ?,\n"
			+ "       t.upd_date = ? where t.country_id = ?";

	public static final String Q_UPDATE_CITY = "update info.cities t set\n"
	...
	public static final String Q_UPDATE_CITY_STATUS = "update info.cities t set\n"
			+ "       t.upd_user = ?,\n"
			+ "       t.upd_date = ?,\n"
			+ "       t.deleted = ? where t.city_id = ?";

	public static final String Q_UPDATE_STREET_TYPE = "update info.street_types t set t.street_type_name_geo = ?, t.street_type_name_eng = ?, t.upd_user = ? where t.street_type_id = ? ";

	public static final String Q_UPDATE_STREET_TYPE_STATUS = "update info.street_types t set t.deleted = ?, t.upd_user = ? where t.street_type_id = ? ";

	public static final String Q_UPDATE_STREET_DESCR = " update info.street_descr t set t.street_descr_name_geo = ?, t.street_descr_name_eng = ?, t.upd_user = ? where t.street_descr_id = ? ";

	public static final String Q_UPDATE_STREET_DESCR_STATUS = "update info.street_descr t set t.deleted = ?, t.upd_user = ? where t.street_descr_id = ? ";

	public static final String Q_UPDATE_STREET = " update info.streets t set\n"
			+ "        t.city_id = ?,\n" + "        t.street_name_geo = ?,\n"
			+ "        t.street_location_geo = ?,\n"
			+ "        t.upd_user = ?,\n" + "        t.record_type = ?,\n"
			+ "        t.descr_id_level_1 = ?,\n"
	...
	public static final String Q_UPDATE_STREET_STATUS = "update info.streets t set t.deleted = ?, t.upd_user = ? where t.street_id = ? ";

	// deletes
	public static final String Q_DELETE_PERS_NOTES = "delete from info.log_personell_notes t where t.note_id = ?";

	public static final String Q_DELETE_PHONES_BY_ABONENT = "delete from info.abonent_to_phones t where t.abonent_id = ? ";

	public static final String Q_DELETE_PERSONNEL = " delete from info.personnel_to_access t where t.personnel_id = ? ";

	public static final String Q_DELETE_STREET_DISCTRICTS_BY_STREET_ID = " delete from info.street_district t where t.street_id = ? ";

	public static final String Q_ROLLBACK_UNAPPROVED_MESSAGES = "update info.call_center_queue t set t.status=1 ,t.upd_date=sysdate where t.status=2 and t.upd_date<=sysdate-1/24/4";
	

	public static final String Q_UPDATE_MAIN_SERVICE_SORT = " update info.main_services t set t.priority = ?,t.upd_user = ?,t.upd_date = ? where t.main_id = ? ";

	public static final String Q_UPDATE_MAIN_SERVICE_DEL_STAT = " update info.main_services t set t.deleted = ?,t.upd_user = ?,t.upd_date = ? where t.main_id = ? ";
	
	public static final String Q_UPDATE_MAIN_SERVICE_HIST = " update info.main_services t set t.upd_user = ?,t.upd_date = ? where t.main_id = ? ";

	public static final String Q_UPDATE_MAIN_ORG_STATUS = " update info.main_orgs t set t.statuse = ? where t.main_id = ? ";
