...
import com.info08.billing.callcenter.client.dialogs.discovery.DlgAddEditDiscoveryType;
...
public class TabDiscoveryTypes extends Tab {
	...
	public TabDiscoveryTypes() {
		try {
			...
			listGrid.setFields(discover_type, rec_date, rec_user, upd_date,
					upd_user);
	...
	private void search() {
		try {
			...
			if (discover_type != null && !discover_type.trim().equals("")) {
				criteria.setAttribute("discover_type", discover_type);
			}
	...
	private void changeStatus(ListGridRecord listGridRecord, Integer deleted) {
		try {
			...
			record.setAttribute("discover_type_id",listGridRecord.getAttributeAsInt("discover_type_id"));
			
			DSRequest req = new DSRequest();

			req.setAttribute("operationId", "updateDiscoverTypeStatus");
