...
public class DlgViewWeather extends MyWindow {
	...
	protected boolean smsSend = false;
	private ToolStripButton sendSMS;

	public DlgViewWeather() {
		super();
		setTitle(CallCenterBK.constants.weather());
		...
		sendSMS = new ToolStripButton(CallCenterBK.constants.smsWeather(),
				"sms.png");
	...
	private void destroyDlg() {
		try {
			final DlgViewWeather dlgViewIndex = this;
	...
	private void sendExactTimeSMS() {
		try {
			...
			recordParam
					.setAttribute("service_id", Constants.serviceWeatherInfo);
			recordParam
